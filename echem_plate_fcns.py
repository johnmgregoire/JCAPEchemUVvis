...
def readechemtxt(path):
    ...
    for count, l in enumerate(lines):
        if l.startswith('%'):
            ...
            break
    z=[map(float, l.split('\t')) for l in lines[count:]]
    for k, arr in zip(d['column_headings'], numpy.float32(z).T):
        d[k]=arr
    return d
    

def readopttxt(path, headingsind=7):
    f=open(path, mode='r')
    lines=f.readlines()
    f.close()
    d={}
    z=[]
    column_headings=lines[headingsind].split('\t')
    column_headings=[s.strip() for s in column_headings]
    z=[map(float, l.split('\t')) for l in lines[headingsind+1:]]
